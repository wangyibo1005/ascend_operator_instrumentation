...
7. **Profile 测试脚本分叉（推荐，不污染原 UT）**
   ...
   - **禁止**：为适配 profiling **直接修改**原 `test_<op>.py` 或原 `*_sample.py`（例如改解包个数、塞 `trace_utils`）。原 UT / 原 sample 应保持与插桩前一致的调用与返回值约定，避免 CI 与历史脚本被动升级。
   - **推荐**：在副本 `test_<op>_profile.py`（及可选的 `*_sample_profile.py`）中完成「多接一个 profiling」「`trace_utils` 落盘」等改动；若需同时保留旧 arity 的 Python API，在 `pybind`/算子侧增加独立入口（例如 `fused_deep_moe_with_profiling`，参数与 `fused_deep_moe` 相同、返回值多一个 profiling 张量），原 `fused_deep_moe` 仍返回插桩前的主输出个数。
   - **命名与位置**：
   ...
   - **拷贝后必改内容**：
     - 在 **profile 副本** 中，将所有 `torch.ops.<lib>.<op>(...)`（或上述「`_with_profiling`」独立入口）的返回值解包在**原有主输出之后多接一个** profiling 张量，例如 fused_deep_moe：`..., profiling = torch.ops.umdk_cam_op_lib.fused_deep_moe_with_profiling(...)`（主输出个数因算子而异，以 `op_host` 注册为准）。
   ...
   - **与工具链对接（参考 `umdk_trace` 示例）**：部署后的 `build/.../trace_utils.py` 提供 `get_enable_moe_profiling()`（从头文件读 `ENABLE_MOE_PROFILING`）和 `save_profiling_data`。典型写法见 `umdk_trace/src/cam/examples/fused_deep_moe_sample.py`（逻辑应落在 **profile 副本** 或独立 `FusionOpWithProfile`，不要改原 sample 的 arity）：
     - `FusionOpWithProfile._apply_ops` 内对 `fused_deep_moe_with_profiling`（或等价入口）解包「原主输出 + profiling」并 `return (..., profiling)`。
   ...
   - **本仓库**：`test_fused_deep_moe.py` 负责 **算子数值 / 融合路径与 baseline 对比等正确性**，不要在其中增加 profiling 解包。`test_fused_deep_moe_profile.py` 仅作 **工具链与 `fused_deep_moe_with_profiling` 等分叉 API** 的轻量检查（`trace_utils`、`point_map.json`）；完整「跑算子 + 落盘 profiling」可在该文件中按同样模式扩展 `FusionOpWithProfile`，并对 `import trace_utils` 做容错。
